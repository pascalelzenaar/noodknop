        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <table class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            <td width="20%">Personeelsnummer</td>
                            <td width="70%">Naam</td>
                            <td></td>
                            <td></td>
                        </tr>
                    ...
                    <tbody>
                        @foreach ($users as $user)
                            <tr>
                                <td>{{ $user->pers_id }}</td>
                                <td>{{ $user->name }}</td>
                                <td><a href="{{ route('show-user', ['id' => $user->id]) }}" class="btn btn-warning"><i class="material-icons">mode_edit</i></button></td>
                                <td>
                                    <form action="{{ route('delete-groep', ['id' => $user->id]) }}" method="post">
                                        {{ method_field('DELETE') }}
                                        {{ csrf_field() }}
                ...
                <div id="createGroup" class="modal fade">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <form action="{{ route('create-groep', ['id' => $user->id]) }}" method="POST">
                                {{ csrf_field() }}
